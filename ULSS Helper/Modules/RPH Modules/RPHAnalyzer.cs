...
public class RPHAnalyzer
{
    internal static async Task<RPHLog> Run(string attachmentUrl)
    {
        ...
        {
            var line = lineReader;
            var allrounder = new Regex(@".+LSPD First Response: (\W*\w*\W*\w*\W*\w*), Version=(.+), Culture=\w+, PublicKeyToken=");
            var allmatch = allrounder.Match(line);
            if (allmatch.Success)
            {
                if (line.Length > 1 && unsorted.All(plugin => plugin.Name != allmatch.Groups[1].Value))
                {
                    var temp = new Plugin();
                    temp.Name = allmatch.Groups[1].Value;
        ...
        foreach (Match match in plugCausingCrashMatch)
        {
            for (int i = match.Groups.Count; i > 0; i--)
            {
                ...
                {
                    if (!log.CausedCrash.Any(x => x.Name.Equals(capture.Value)))
                    {
                        foreach (var plugin in pluginData.Where(plugin => plugin.Name.Equals(capture.Value)))
                        {
                            if (!log.CausedCrash.Contains(plugin))
                            {
                                log.CausedCrash.Add(plugin);
                            }
                        }
                    }
                }
    ...
    timer.Stop();
        ...
        Console.WriteLine($"Caused a Crash: {log.CausedCrash.Count}");
